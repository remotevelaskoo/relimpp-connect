...
| **PurchaseRequestAttachment** | `fileName` (nome original), `mimeType`, `size` (bytes), `storagePath` (caminho relativo em disco), `uploadedBy?` | N—1 PurchaseRequest (`onDelete: Cascade`) | Anexos (Especificação, seção 8.1). **Armazenamento em disco local** (`backend/uploads/purchase-requests/<requestId>/`, gitignored) — não é S3/blob storage; funciona neste ambiente mas não sobrevive a um deploy sem disco persistente (ex.: a maioria das PaaS serverless). Nome do arquivo em disco é um UUID gerado no upload (não o nome original) para evitar colisão e path traversal. Sem limite de tipo de arquivo; limite de tamanho (15MB) é aplicado na camada HTTP (Multer), não no schema. |
...
| `20260722025058_purchase_request_attachments` | Tabela `PurchaseRequestAttachment`. |
